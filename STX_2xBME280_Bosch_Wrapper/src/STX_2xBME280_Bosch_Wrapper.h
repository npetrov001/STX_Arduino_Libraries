...
#include "Bosch_BME280_incMod/Bosch_BME280_Arduino.h"


/******************************************************************************
STX_2xBME280_Bosch_Wrapper
Reads two BME280 sensors using the Bosch_BME280_Arduino library.
...
class STX_2xBME280_Bosch_Wrapper
{
public:
...
protected:
    ...
    uint32_t _startSensor(BME::Bosch_BME280& sensor, uint8_t addr);
    uint8_t  _readChipID(uint8_t addr);
    uint32_t _readBoschID(uint8_t addr);
    void     _readSensor(uint32_t id, BME::Bosch_BME280& sensor, float& T, float& H, float& P);
...
public:
    // Bosch BME280 sensor objects
    BME::Bosch_BME280 sensor1 {0x76};
    BME::Bosch_BME280 sensor2 {0x77};
};
